...
router.get("/firm-storage-info", async (req, res, next) => {
  try {
    ...
    try {
      ...
      return res
        .status(400)
        .json({ status: "error", message: "Invalid firm ID" });
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        storageAvailableGB: isUnlimited
          ? "Unlimited"
          : parseFloat((storageLimitGB - storageUsedGB).toFixed(4)),
...
router.post("/sync-storage", async (req, res, next) => {
  try {
    ...
    const usageGB =
      usage.length > 0 ? usage[0].totalSize / (1024 * 1024 * 1024) : 0;
